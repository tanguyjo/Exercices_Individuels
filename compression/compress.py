...
texte2 = ['hey','my','name','is','Tanguy']

def split_text(texte):
    return texte.split(' ')

print(split_text(texte1))


def join_text(text):
    return ' '.join(text)

print(join_text(texte2))
...
def dictionnaires(texte):
    result = []
    for mot in texte:
        if mot in dicoliste:
            result.append(dicoliste[mot])
        else:
            result.append(mot)
    return result
liste =['mais', 'ceci', 'est', 'un', 'long', 'faux-texte']
print(dictionnaires(liste))